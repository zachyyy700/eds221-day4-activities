...
force <- function(mass, acceleration) {
  if (!is.numeric(mass) | !is.numeric(acceleration)) {
    stop("Isaac Newton uses numbers when doing math, you should too")
  } else {
...
fish_weight <- function(fish_name, tot_length) {
  if (!fish_name %in% fish_parms$common_name) {
    stop("You had three fish to choose, that's not one of them")
  }
  if (tot_length < 0) {
    stop("Your fish is made of anti-matter, try again")
  }
